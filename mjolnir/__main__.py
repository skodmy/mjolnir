...
class HttpPoster(Thread):
    def __init__(self, host, port, use_tor):
        super().__init__()
        self.host = host
        self.port = port
    ...
    def _send_http_post(self):
        """
        Sending post requests with some garbage.
        """
        global stop_now

        self.socks.send(bytes("POST / HTTP/1.1\r\n"
                              "Host: %s\r\n"
                              "User-Agent: %s\r\n"
                              ...
                              (self.host, random.choice(user_agents)), encoding='utf-8'))
